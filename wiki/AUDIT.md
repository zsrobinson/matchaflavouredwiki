...
| 8 | Complete disabled-vanilla list (JeremyVyska, 356 entries) | **Closed.** `extract.py` records every recipe the `pack.mcmeta` filter hides (`blocked_recipes`: type, recipe book tab, output) and the advancement folders (`blocked_advancements`); `generate.py` writes `Template:Data/Blocked vanilla` (360 recipes in 327 rows, grouped by station and recipe book tab, each with the stations that still make the item, plus the five advancement tabs) and count templates. Removed features transcludes it and keeps the prose that explains why. The filter's 366 distinct entries (401 with repeats) include `turtle_shell.json`, which vanilla 26.2 no longer has, so it isn't listed. Gamerule changes and replaced loot or structures aren't filter entries and stay in prose. | **Tooling** (generator output missing). |
...
| 10 | Progression-safe spoiler mode (peterbax, Evansch0, JeremyVyska) | **Closed.** A "hide spoilers" toggle beside dark mode (off by default) hides what `{{Spoiler}}` covers and hidden-advancement rows until clicked; the rule is in STYLE.md ("Spoilers"). | **Feature gap**. |
...
| 13 | "Sleep sometimes doesn't work after the first join until `/reload`" (AriesAlex, citing the Modrinth description) | **Closed, not added.** The Modrinth description is now a watched source (`sources/modrinth_project.md`), and its current text doesn't make this claim. The 1.10 release notes say "Beds now work on first load-in", which Sleeping's history already records, so the claim describes a bug fixed in 1.10. Reading the description also added: the inhabited house as the pack's only "explicit horror" (Abandoned village), clay fetishes shown as statues in item frames (Clay Fetish), and the answers on mod integration and vanilla textures (Upcoming features). Everything else in it (tips, installation, credits) was already on pages via the repository's README. | **Source not considered**: the Modrinth project description (and gallery) isn't fetched. |
...
3. **Generator coverage**: no redirects for advancement titles, plurals, British spellings or internal model ids; no table for `blocked_vanilla` (now `Template:Data/Blocked vanilla`).
...
1. **Watch every official channel** (the coordinator reports this is now in place for YouTube):
   ...
   - Snapshot the Modrinth project description and gallery captions (`/v2/project/matcha-flavoured`) into `source/modrinth_project.md`, and diff it daily. Treat it as a primary source, like the release notes. **Done:** `tools/fetch_modrinth_project.py` saves it to `sources/modrinth_project.md` (committed, so the diff survives between sessions), `check_upstream.py` reports `description_changed`, `update_report.py --description-only` writes the checklist line, and pages cite it with `{{Cite Modrinth}}`.
