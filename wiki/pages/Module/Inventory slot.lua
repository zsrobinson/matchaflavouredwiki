...
local function makeItem( frame, args )
	...
	-- vanilla items with no page on this wiki link to minecraft.wiki (list generated by tools/generate.py)
	if link and ( args.link or '' ) == '' then
		local offsite = mw.loadData( 'Module:Inventory slot/Offsite' )[link]
		if offsite then
			link = 'https://minecraft.wiki/w/' .. mw.uri.encode( offsite, 'WIKI' )
		end
	end
	...
	if num and num > 1 and num < 1000 then
		local external = link and link:find( '^https?://' )
		if link then
			item:wikitext( external and '[' or '[[', link, external and ' ' or '|' )
		end
		local number = item
			:tag( 'span' )
