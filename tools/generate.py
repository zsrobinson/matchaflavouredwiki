...
def release_name(version):
    """The name the release of a pack version has on Modrinth: pack.mcmeta says "1.12.2" for 1.12.2-beta.
    The newest release notes (source/changelogs, fetch_sources.sh) for that version, else the version itself."""
    names = []
    for fn in sorted(glob.glob(os.path.join(ROOT, 'source', 'changelogs', '*.md'))):
        name = os.path.basename(fn)[len('YYYY-MM-DD_'):-len('.md')]
        if not name.endswith('-rp') and (name == version or name.startswith(version + '-')):
            names.append(name)
    return names[-1] if names else version
...
def main():
    ...
    ver = release_name(ver.group(1)) if ver else '?'
    write('Template', 'Data/Current version', '<includeonly>%s</includeonly><noinclude>The Modrinth release of the pack version in <code>pack.mcmeta</code> at the synced commit (1.12.2 was released as 1.12.2-beta). Generated.</noinclude>' % ver)
    write('Template', 'Data/Stable version', '<includeonly>%s</includeonly><noinclude>The newest full release on Modrinth (not an alpha or beta) up to the current version; the current version itself once it is one. Generated.</noinclude>' % stable_release(ver))
