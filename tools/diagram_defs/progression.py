...
def equipment_tiers():
    ...
    for key, label, x, _ in main:
        f.node(key, x, y0, label, w=104, icon='%s Pickaxe' % key, bold=True)
    for (a, _, _, _), (b, _, _, made) in zip(main, main[1:]):
        f.arrow(a, b, made_in(made))
    # alloys, from the smithing recipes that take each base chestplate
    for key, _, x, _ in main[1:]:
        results = alloys_on('%s Chestplate' % key)
        xs = [x] if len(results) == 1 else [x - 55, x + 55]
        for (result, alloy), ax in zip(results, xs):
            name = result.replace(' Chestplate', '')
            f.node(name, ax, y1, name, w=104, icon='%s Pickaxe' % name, fill='@amber_soft', edge='@amber')
    ...
    f.node('Golden', 62, y1, 'Gold', w=104, icon='Golden Pickaxe', sub='side tier', dash='5 4')
    svg.h = y1 + 52
    return svg
