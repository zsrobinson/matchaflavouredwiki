...
- **The Modrinth project page is a primary source too:** its description and gallery captions are the
  developer's own words (`sources/modrinth_project.md`, saved by `tools/fetch_modrinth_project.py` and
  watched daily by `check_upstream.py`). Cite it with `{{Cite Modrinth|date=…|quote=…}}` (`gallery=<title>`
  for a caption). As with the videos, the code wins where they disagree.
...
- **Modrinth project API** (`/v2/project/matcha-flavoured`) provides the description and gallery captions,
  committed as `sources/modrinth_project.md` so `git diff` shows each edit. Downloads, dates and the version
  list are left out of it, so it changes only when the developer's text does.
